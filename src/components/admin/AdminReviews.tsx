...
import { Star, Check, EyeOff, RefreshCw, AlertCircle, Trash2, Loader2 } from "lucide-react";
...
function StarDisplay({ count }: { count: number }) {
  return (
    <span className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map((s) => (
        <Star key={s} size={14} fill={s <= count ? "#f59e0b" : "none"} stroke={s <= count ? "#f59e0b" : "#d1d5db"} strokeWidth={1.5} />
      ))}
    </span>
  );
}
        ...
        <div className="space-y-3">
          {filtered.map((review) => (
            <motion.div
              ...
              className={`bg-card rounded-xl border p-4 flex flex-col sm:flex-row sm:items-start gap-3 ${
                review.is_approved ? "border-green-200" : "border-amber-200"
              }`}
            >
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1 flex-wrap">
                  ...
                  <span className={`text-xs font-body px-2 py-0.5 rounded-full ml-auto ${
              ...
              <div className="flex gap-2 shrink-0">
                {!review.is_approved ? (
                  ...
                  >
                    {actionId === review.id ? <Loader2 size={13} className="animate-spin" /> : <Check size={13} />}
                    Tornar Visível
                  </Button>
                ) : (
                  <Button
                  ...
                  >
                    {actionId === review.id ? <Loader2 size={13} className="animate-spin" /> : <EyeOff size={13} />}
                    Ocultar
                  </Button>
                )}
                <Button
                ...
                >
                  {actionId === review.id ? <Loader2 size={13} className="animate-spin" /> : <Trash2 size={13} />}
                  Excluir
                </Button>
              </div>
            </motion.div>
